...
const formatCurrency = (amount) => {
    if (amount === undefined || amount === null) {
      return "0 đ"; // Hoặc giá trị mặc định khác tùy thuộc vào yêu cầu của bạn
    }
...
const Payment = () => {
  ...
  const createOrder = (data, actions) => {
    ...
    const exchangeRate = 0.00004; // 1 VND = 0.00004 USD
    const totalPriceInUSD = (orderData?.totalPrice * exchangeRate).toFixed(2);
  ...
  return (
    <div className="w-full bg-[#fff] rounded-md p-5 pb-8 shadow-md">
      <div className="flex justify-between">
        <h3 className="text-[16px] font-[400] text-[#000000a4]">Tạm tính:</h3>
      ...
      <div className="flex justify-between">
        ...
        <h5 className="text-[18px] font-[600]">{formatCurrency(shipping)}</h5>
      </div>
      <br />
